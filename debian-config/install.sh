...
neovim() {
  ...
  echo $user_password | sudo -S mv /tmp/nvim.appimage /usr/local/bin
}
...
#   cd $libvirt
#   meson setup build -Dsystem=true -Ddocs=disabled -Dprefix=/usr/local -Dtests=disabled
#   ninja -C build

#   echo $user_password | sudo -S ninja -C build install
#   echo $user_password | sudo -S ldconfig

#   id libvirt-qemu || echo $user_password | sudo -S useradd libvirt-qemu
#   echo $user_password | sudo -S groupadd -f libvirt-qemu
#   echo $user_password | sudo -S groupadd -f libvirt
#   echo $user_password | sudo -S groupadd -f libvirtd
#   echo $user_password | sudo -S usermod -aG libvirt-qemu libvirt-qemu
#   echo $user_password | sudo -S usermod -aG libvirt-qemu,libvirtd,libvirt ab55al
# }
